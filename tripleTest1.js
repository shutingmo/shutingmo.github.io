...
function runClarifai(arrayImagesToPass)
{
    console.log("we're in runClarifai");
  for(x in arrayImagesToPass)
  {
      appClarifai.models.predict("c0c0ac362b03416da06ab3fa36fb58e3", arrayImagesToPass[x]).then(
        function(response) {
...
app.get('/test', function(req, res) {
  ...
  loadURL(['https://www.billboard.com'], function(nestedImgArrs) {

    res.status(200).send("all done loading urls")
  })
});
// pagesToVisit.push(START_URL);
// crawl();
...
function loadURL(arrayPagesToPass, callback)
{
    ...
    for(var i = 0; i < arrayPagesToPassLength; i++)
    {
        console.log("-----------------------------------------------");
        ...
        /*
            ASYNC
            scrape(fn: argument of type function)

            access web page, when we get results, call fn()
        */
      var done = false
        ...
        //     // imagesToPass[i] = image.address;
        //     imagesToPass.push(image.address);
        //     console.log('stats', imagesToPass.length, arrayPagesToPassLength)
        //     // if (imagesToPass.length == arrayPagesToPassLength) {
        //     //     console.log("We are done done, time for callbacks")
        //     // }

        //     // console.log("the images to pass array has " + JSON.stringify(imagesToPass));
    ...
    console.log("images to pass are " + JSON.stringify(imagesToPass));
...
function crawl() {
    ...
    console.log("We've seen this page")
